...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        # --- Boats ---
        boats = conn.execute(
            "SELECT id, name, group_name FROM boats WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for b in boats:
            assignments = conn.execute(
        ...
        # --- Picture boats ---
        pboats = conn.execute(
            "SELECT id, name, group_name FROM picture_boats WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for b in pboats:
            assignments = conn.execute(
        ...
        # --- Security boats ---
        sboats = conn.execute(
            "SELECT id, name, group_name FROM security_boats WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for b in sboats:
            assignments = conn.execute(
        ...
        vehicles = conn.execute(
            "SELECT id, name, type FROM transport_vehicles WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for v in vehicles:
            assignments = conn.execute(
        ...
        helpers = conn.execute(
            "SELECT id, name, role, group_name FROM helpers WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for h in helpers:
            assignments = conn.execute(
        ...
        # --- Guards (camp workers) ---
        # Note: guard_camp_assignments.helper_id references guard_camp_workers.id
        guards = conn.execute(
            "SELECT id, name, role, group_name FROM guard_camp_workers WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for g in guards:
            assignments = conn.execute(
            ...
            resources.append({
                ...
                'subgroup': g['group_name'] or g['role'] or 'Guards',
                'assignments': [_timeline_assign(a) for a in assignments]
            })

        # --- Locations ---
        # location_schedules has a single `status` column with values 'P' (prep) / 'F' (filming) / 'W' (wrap).
        locations = conn.execute(
            "SELECT id, name, location_type FROM locations WHERE production_id=? AND deleted_at IS NULL",
            (prod_id,)
        ).fetchall()
        for loc in locations:
            schedules = conn.execute(
                "SELECT id, date, status FROM location_schedules WHERE location_id=? AND status IS NOT NULL AND status != ''",
                (loc['id'],)
            ).fetchall()
            loc_assignments = []
            for s in schedules:
