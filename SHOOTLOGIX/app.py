...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        for loc in locations:
            ...
            # status in {'P','F','W'}. Group by date so all phases for a given
            # day collapse to a single assignment with phases like "P/F".
            schedules = conn.execute(
            ...
            by_date = {}
            for s in schedules:
                if s['status'] not in ('P', 'F', 'W'):
                    continue
                slot = by_date.setdefault(s['date'], {'id': s['id'], 'phases': []})
                slot['phases'].append(s['status'])
            loc_assignments = [
                {
