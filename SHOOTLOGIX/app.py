...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        for loc in locations:
            schedules = conn.execute(
                "SELECT id, date, status FROM location_schedules WHERE production_id=? AND location_name=?",
                (prod_id, loc['name'])
            ).fetchall()
            # Group schedules by date to combine P/F/W phases
            by_date = {}
            for s in schedules:
                d = s['date']
                if d not in by_date:
                    by_date[d] = {'id': s['id'], 'phases': []}
                by_date[d]['phases'].append(s['status'])
            loc_assignments = []
