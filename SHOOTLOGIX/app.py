...
def api_duplicate_picture_boat(pb_id):
    ...
    with get_db() as conn:
        new_row = conn.execute("SELECT * FROM boats WHERE id=? AND category='picture'", (new_id,)).fetchone()
    return jsonify(dict(new_row)), 201
...
def api_duplicate_security_boat(sb_id):
    ...
    with get_db() as conn:
        new_row = conn.execute("SELECT * FROM boats WHERE id=? AND category='security'", (new_id,)).fetchone()
    return jsonify(dict(new_row)), 201
...
def _auto_match_boat_photos():
    ...
    with get_db() as conn:
        ...
        # --- PICTURE BOATS (from boats table, category='picture') ---
        ...
        # --- SECURITY BOATS (from boats table, category='security') ---
...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        # --- Picture boats ---
        pboats = conn.execute("SELECT id, name, group_name FROM boats WHERE production_id=? AND category='picture' AND deleted_at IS NULL", (prod_id,)).fetchall()
        for b in pboats:
            assignments = conn.execute(
        ...
        # --- Security boats ---
        sboats = conn.execute("SELECT id, name, group_name FROM boats WHERE production_id=? AND category='security' AND deleted_at IS NULL", (prod_id,)).fetchall()
        for b in sboats:
            assignments = conn.execute(
