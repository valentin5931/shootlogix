...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        for loc in locations:
            # location_schedules stores one row per (location, date) with a single
            # status code: 'P' (prep) / 'F' (filming) / 'W' (wrap). Group rows by
            # date so the timeline shows the combined phases for that day.
            schedules = conn.execute(
            ...
            loc_assignments = [
                {
                    ...
                    'status': 'confirmed', 'phases': '/'.join(entry['phases'])
                }
