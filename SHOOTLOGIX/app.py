...
def api_timeline(prod_id):
    ...
    with get_db() as conn:
        ...
        # --- Locations ---
        # location_schedules.status is a single-letter phase ('P' | 'F' | 'W').
        # locations has no 'site' column; use 'location_type' as the subgroup label.
            ...
            for s in schedules:
                if s['status'] in ('P', 'F', 'W'):
                    loc_assignments.append({
                        ...
                        'status': 'confirmed', 'phases': s['status']
                    })
            resources.append({
