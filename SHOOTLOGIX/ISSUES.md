...
## ~~[P0] Timeline API crash — invalid column references~~ FIXED 2026-04-02
- **Fixed in**: fix/2026-04-02-timeline-api-crash-site-column
- **Details**: `api_timeline()` referenced non-existent `site` column in `locations` table and `prep`/`filming`/`wrap` columns in `location_schedules` table. Fixed to use `location_type` and `status` respectively.
