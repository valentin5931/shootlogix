...
## [P0] FIXED — Picture boats state variable mismatch in _reloadCurrentTab and _findAssignment
- **Discovered**: 2026-04-01
- **Fixed**: 2026-04-01 (branch: fix/2026-04-01-picture-boats-state-variable-mismatch)
- **Symptoms**: After pull-to-refresh on Picture Boats tab, functions and assignments disappear. Day overrides on picture boat assignments fail silently.
- **Root cause**: `_reloadCurrentTab()` stored data in `state.pbFunctions`/`state.pbAssignments` but rest of code uses `state.pictureFunctions`/`state.pictureAssignments`. `_findAssignment()` also searched wrong variable.
- **Files involved**: `static/app-monolith.js` (lines 13002, 3522)
