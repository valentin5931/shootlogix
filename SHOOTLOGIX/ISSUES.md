...
- **Resolved**: 2026-04-01 by commit 2a93828 (#32) — `renderFleetUnified` / `renderCrewUnified` now clear `active` from all related sub-panels before activating the target, fixing the panel-stacking visual bug. Sub-nav uses inline `onclick="App.fleetSetSubTab(...)"` so DOM reparenting does not strip handlers.
