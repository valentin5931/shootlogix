...
- **Discovered**: 2026-03-22
- **Fixed**: 2026-04-02 — Branch `fix/2026-04-02-seed-picture-security-boats`
- **Root cause**: `_seed_picture_boats()` and `_seed_security_boats()` only seeded boat functions, not actual boat entries. The 46 fleet boats existed only in the `boats` table.
- **Fix**: Added `_seed_picture_boats_from_fleet()` (46 boats) and `_seed_security_boats_from_fleet()` (4 safety boats) migration functions in `data_loader.py`
