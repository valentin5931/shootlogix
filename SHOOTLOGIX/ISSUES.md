...
- **Symptoms**: `/api/productions/1/timeline` returns 500 Internal Server Error. Timeline tab is completely broken.
- **Root cause**: `api_timeline()` in `app.py` queried `locations.site` (doesn't exist, should be `location_type`) and `location_schedules.prep/filming/wrap` (don't exist, should be `status`).
- **Fix**: Branch `fix/2026-04-03-timeline-api-crash-site-column` — corrected column names to match actual schema.
