...
## [FIXED] Fleet/Crew sub-tab FAB (+ button) hidden
- **Discovered**: 2026-03-22
- **Fixed**: 2026-03-22 — Branch `fix/2026-03-22-fleet-crew-fab-hidden`
- **Fix**: Added `_fabEffectiveTab()` to resolve `fleet`/`crew` tab to their active sub-tab for FAB lookup

## [P1] Fleet sub-nav layout shifts on sub-tab switch
- **Discovered**: 2026-03-22
- **Symptoms**: The fleet sub-nav element is moved between view panels via `prepend()` on each sub-tab switch, which may cause brief layout shifts.
- **Likely cause**: Sub-nav is a single DOM element that gets re-parented on every switch.
- **Files involved**: `static/app-monolith.js` (renderFleetUnified, renderCrewUnified)
- **Estimated effort**: Quick — could keep sub-nav in a fixed container outside view panels
