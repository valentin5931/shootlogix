...
- **Fixed**: 2026-03-22 — Branch: fix/2026-03-22-fleet-crew-subtab-panel-switching
- **Root cause**: `renderFleetUnified()` and `renderCrewUnified()` only hid their placeholder panel, leaving the previous sub-tab panel still active. Also `setTab()` briefly activated the placeholder before the render function overrode it.
- **Fix**: Hide ALL related sub-panels before showing target; skip placeholder activation in `setTab()`
