...
## [P0] ~~Timeline API crash — no such column: site~~ FIXED 2026-04-09
- **Discovered**: 2026-04-09
- **Symptoms**: `/api/productions/1/timeline` returned 500 with `sqlite3.OperationalError: no such column: site`
- **Root cause**: Three invalid column references in `api_timeline`: `locations.site` (→ `location_type`), `location_schedules.prep/filming/wrap` (→ `status`), `guard_camp_assignments.worker_id` (→ `helper_id`)
- **Fixed in**: Branch `fix/2026-04-09-timeline-crash-no-site-column`
