...
## ~~[P0] Fleet/Crew sub-tab event handlers may not fire on cloned DOM~~ — FIXED 2026-04-15
- Resolved by making the sub-nav static HTML inside each fleet/crew panel.
  `renderFleetUnified` / `renderCrewUnified` no longer move DOM nodes; they only
  toggle the `.active` class on `[data-fleet-sub]` / `[data-crew-sub]` buttons.
  See CHANGELOG 2026-04-15.
