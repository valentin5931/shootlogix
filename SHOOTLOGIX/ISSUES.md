...
## [P0] [FIXED 2026-04-09] Timeline API crash — no such column: site
- **Discovered**: 2026-04-09
- **Symptoms**: Clicking the Timeline tab caused a 500 error. The `/api/productions/{id}/timeline` endpoint crashed with `sqlite3.OperationalError: no such column: site`.
- **Root cause**: SQL queries in the timeline endpoint referenced non-existent columns (`site` in locations, `prep/filming/wrap` in location_schedules). The actual schema uses `location_type` and `status` respectively.
- **Fix**: Updated column references and added soft-delete filtering. See CHANGELOG 2026-04-09.
