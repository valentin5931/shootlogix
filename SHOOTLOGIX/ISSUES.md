...
- **Discovered**: 2026-03-22
- **Fixed**: 2026-04-01 — Added fallback to query `boats` table by category when dedicated tables are empty
- **Symptoms**: `/api/productions/1/picture-boats` returned `[]`. Now returns 46 boats.
- **Remaining**: Security boats still empty because no boats have `category='security'` — users need to re-categorize safety/security boats (EVAC, SAFETY, etc.) via the UI or a data migration.
