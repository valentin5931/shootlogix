...
## [FIXED] Pull-to-refresh writes to wrong state keys for Picture Boats tab
- **Discovered**: 2026-04-01
- **Fixed**: 2026-04-01 (fix/2026-04-01-reload-tab-state-keys)
- **Status**: Resolved — `_reloadCurrentTab()` now uses correct state keys (`pictureFunctions`, `pictureAssignments`) and `?context=boats` filter. `_findAssignment()` now searches the correct state arrays.

## [FIXED] Fleet/Crew sub-tab event handlers may not fire on cloned DOM
- **Discovered**: 2026-03-22
- **Fixed**: 2026-03-23 (sub-nav layout and CSS vars fix)
- **Status**: Resolved — sub-nav injection works, layout overflow fixed with `--subnav-bar-h` CSS var
