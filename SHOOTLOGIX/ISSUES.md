...
## [FIXED] [P1] Checklist tab completely non-functional
- **Discovered**: 2026-04-02
- **Fixed**: 2026-04-02
- **Symptoms**: Clicking the Checklist tab showed nothing — no data loaded, generate/toggle did nothing
- **Root cause**: Checklist JS functions used `state.production.id` (never set) instead of `state.prodId`
- **Fix**: Replaced all 6 references in `app-monolith.js`
...
## [UPDATED] [P1] Helpers list is empty (Transport resolved)
- **Discovered**: 2026-03-22
- **Updated**: 2026-04-02
- **Symptoms**: `/api/productions/1/helpers` returns `[]` (but helper-assignments has 73 entries with null helper_id). Transport is now working: `/api/productions/1/transport-vehicles` returns 14 vehicles.
- **Likely cause**: Helpers need to be created manually by users; the 73 helper_assignments are placeholder function slots without assigned workers.
- **Files involved**: `database.py`
- **Estimated effort**: Quick — user needs to add helper data through the UI
