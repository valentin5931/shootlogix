...
## ~~[P0] Timeline API crash — no such column: site~~
- **Discovered**: 2026-04-13
- **Status**: FIXED (2026-04-13, branch fix/2026-04-13-timeline-api-crash-no-column-site)
- **Symptoms**: `/api/productions/1/timeline` returns 500 with `sqlite3.OperationalError: no such column: site`
- **Root cause**: Timeline query referenced `locations.site` (doesn't exist, should be `location_type`) and `location_schedules.prep/filming/wrap` (don't exist, should use `status` column)
- **Fix**: Updated query and logic in `app.py` lines 7892-7914
