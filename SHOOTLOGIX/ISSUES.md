...
## [P0] Timeline API crash — FIXED 2026-04-10
- **Discovered**: 2026-04-10
- **Symptoms**: `/api/productions/1/timeline` returned 500 error (`no such column: site`, then `no such column: prep`)
- **Root cause**: SQL queries referenced non-existent columns (`site` in locations, `prep/filming/wrap` in location_schedules)
- **Status**: **FIXED** in branch `fix/2026-04-10-timeline-api-crash-no-site-column`

## [P1] Checklist API route missing (404)
- **Discovered**: 2026-04-10
- **Symptoms**: `/api/productions/1/checklist` returns 404. The Checklist tab in the UI would fail to load data.
- **Likely cause**: No Flask route has been defined for the checklist endpoint yet
- **Files involved**: `app.py` (needs new route), `static/app-monolith.js` (checklist tab handler)
- **Estimated effort**: Medium — need to define the data model and API endpoint for checklist items
