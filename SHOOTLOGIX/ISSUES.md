...
- **Discovered**: 2026-03-22
- **Fixed**: 2026-04-05 — Branch: fix/2026-04-05-seed-picture-security-boats
- **Root cause**: `_seed_picture_boats()` and `_seed_security_boats()` in `data_loader.py` only created `boat_functions` (roles), not actual boat entities in the `picture_boats` and `security_boats` tables.
- **Fix**: Updated both seed functions to also create boat entities, following the same pattern as `_seed_transport()`. Also added `_seed_security_boats()` call to the existing-production bootstrap path.
