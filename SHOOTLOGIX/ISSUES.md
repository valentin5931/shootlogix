...
## [P0] ~~Checklist tab completely non-functional~~ FIXED 2026-04-14
- **Discovered**: 2026-04-14
- **Symptoms**: Checklist tab shows nothing when opened. "Generate" button does nothing. Checking items does nothing. No error messages displayed.
- **Root cause**: All checklist JS functions used `state.production.id` which was never set. The rest of the app uses `state.prodId`.
- **Fix**: Replaced `state.production` / `state.production.id` with `state.prodId` in `loadChecklist()`, `generateChecklist()`, and `toggleChecklistItem()`.
