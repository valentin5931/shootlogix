...
## [RESOLVED 2026-04-08] Timeline endpoint 500 — schema mismatches in api_timeline
- **Was**: `GET /api/productions/<id>/timeline` crashed with `sqlite3.OperationalError: no such column: site`. Two more latent column/join-key bugs (`location_schedules.prep/filming/wrap`, `guard_camp_assignments.worker_id`) were fixed in the same handler.
- **Resolved by**: PR `fix/2026-04-08-timeline-locations-site-column` — see CHANGELOG entry of 2026-04-08.
