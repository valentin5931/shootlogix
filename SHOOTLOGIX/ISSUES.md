...
## [P0] ~~Checklist tab completely broken — state.production never set~~
- **Discovered**: 2026-04-10
- **Status**: FIXED (2026-04-10, branch fix/2026-04-10-checklist-tab-broken-state-production)
- **Symptoms**: Clicking the Checklist tab showed an empty panel. No data loaded, no error displayed.
- **Root cause**: `loadChecklist()`, `generateChecklist()`, and `toggleChecklistItem()` all used `state.production` (never assigned) instead of `state.prodId`. The guard `if (!state.production) return;` silently exited.
- **Fix**: Replaced all 6 references to `state.production` with `state.prodId` in `static/app-monolith.js`.
